...
contract yDelegatedVault is ERC20 {
    ...
    function debt() public view returns (uint256) {
        ...
        (, uint256 currentBorrowBalance, , , , , , , , ) =
            Aave(getAave()).getUserReserveData(_reserve, address(this));
        ...
        returns (uint256)
    {
        _amount = _amount.mul(getUnderlyingPrice()).div(
            uint256(10)**ERC20(address(token)).decimals()
    ...
    function over(uint256 _amount) public view returns (uint256) {
        ...
        if (_maxSafeETH < _totalBorrowsETH) {
            uint256 _over =
                _totalBorrowsETH.mul(_totalBorrowsETH.sub(_maxSafeETH)).div(
                    _totalBorrowsETH
                );
            _over = _over.mul(uint256(10)**ERC20(_reserve).decimals()).div(
                getReservePrice()
            );
            return _over;
        } else {
    ...
    {
        (, , uint256 _totalBorrowsETH, , uint256 _availableBorrowsETH, , , ) =
    ...
    function availableToBorrowReserve() public view returns (uint256) {
        ...
        if (_available > 0) {
            return
                _available.mul(uint256(10)**ERC20(_reserve).decimals()).div(
                    getReservePrice()
                );
        } else {
            return 0;
