...
contract StrategyDForceDAI {
    ...
    address public constant weth =
        address(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2); // used for df <> weth <> usdc route
    ...
    function deposit() public {
        ...
        uint256 _d = IERC20(ddai).balanceOf(address(this));
        if (_d > 0) {
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint256) {
        uint256 _d = _amount.mul(1e18).div(dERC20(ddai).getExchangeRate());
    ...
    function balanceOfPool() public view returns (uint256) {
        return
            (dRewards(pool).balanceOf(address(this)))
                .mul(dERC20(ddai).getExchangeRate())
                .div(1e18);
    }
